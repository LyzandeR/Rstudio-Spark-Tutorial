...
```{R}
library(DBI)
df_sql <- dbGetQuery(sc, "SELECT * FROM flights WHERE distance > 4800 LIMIT 5")
df_sql
```
...
the latter will also collect. `mutate` instead of `transform` is used in `sparklyr` to add a column.
